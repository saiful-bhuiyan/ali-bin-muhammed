...
<body style="background: #32aba9">
    ...
    <div class="container col-lg-10 rounded-3 text-center" style="background: #4a91bd; ">
      ...
      <div class="row mx-auto text-center mb-3">
        ...
        <!-- Php code of get ID -->
          ...
          <?php
            $query="SELECT MAX(`id`) FROM `customer_setup`";
        ...
        <div class="mb-3 col-lg-6 col-md-6 col-sm-12">
          ...
          <input type="text" class="col-lg-8 col-md-10 col-sm-12 form-control" id="customerName" placeholder=" " maxlength="40">
        </div>
        <div class="mb-3 col-lg-6 col-md-6 col-sm-12">
          <label class="form-label">Customer Mobile :</label><br>
          <input type="text" class="col-lg-8 col-md-10 col-sm-12 form-control" id="customerMobile" placeholder=" " maxlength="20"
                 oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" >
        </div>
        <div class="mb-3 col-lg-6 col-md-6 col-sm-12">
          <label class="form-label">Customer Address :</label><br>
          <input type="text" class="col-lg-8 col-md-10 col-sm-12 form-control" id="customerAddress" placeholder=" " maxlength="40">
        </div>
        <div class="mb-3 col-lg-6 col-md-6 col-sm-12">
          <label  class="form-label">Old Amount :</label><br>
          <input type="text" class="col-lg-8 col-md-10 col-sm-12 form-control" id="oldAmount" placeholder=" " maxlength="20" 
            oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" >
        </div>

        <div class="mb-3" id="insertBtn">
          <button class="btn btn-success" id="cusAdd" type="button">Save</button>
        </div>
      </div>
      </div>
    </div>
    ...
    <div class="container col-lg-10 rounded-3 text-center" style="background: white;">
      ...
      <div class="row overflow-scroll">
        <table id="table" class="table table-striped table-bordered">
          ...
          <tbody>
            <?php
              $sel="SELECT *FROM `customer_setup`";
                ...
                while($row=mysqli_fetch_assoc($q))
                  {
                    ...
                    $a=$row['id'];
                    $b=$row['customer_address'];
                    $c=$row['customer_name'];
                    $d=$row['customer_mobile'];
                    $e=$row['old_amount'];
              ...
              <tr>
                ...
                <td><?php echo $c; ?></td>
                <td><?php echo $d; ?></td>
                <td><?php echo number_format($e,3) ?></td>
              </tr>
              <?php
              $count++;
                }
  ...
  <script>
    ...
    $(document).on('click','#cusAdd',function(){
      $('#cusAdd').attr("disabled", "disabled");
      $('#cusAdd').text("Please Wait !!!");
      ...
      if(fID !="" && customerName !="" && customerMobile !="" && customerAddress !="" && oldAmount != ""){
        $.ajax({
          url: 'csetInsert.php',
          type: "POST",
          data:{
          ...
          success:function(response){
            ...
            var data = $.parseJSON(response);
              if(data.status ==  'success'){
                window.location = 'customer_setup.php';
              }else{
                swal(data.message)
                $('#cusAdd').removeAttr("disabled");
                $('#cusAdd').text("Save");
              }
