...
<body style="background: #32aba9">
    ...
    <div class="container col-lg-10 rounded-3 text-center" style="background: #4a91bd; ">
      ...
      <div class="row mx-auto text-center mb-3">
        ...
        <!-- Php code of get ID -->
          ...
          <?php
            $query="SELECT MAX(`id`) FROM `supplier_setup`";
        ...
        <div class="mb-3" id="insertBtn">
          <button class="btn btn-success" id="supAdd" type="button">Save</button>
        </div>
      </div>
      </div>
    </div>
    ...
    <div class="container col-lg-10 rounded-3 text-center" style="background: white;">
      ...
      <div class="row overflow-scroll">
        <table id="table" class="table table-striped table-bordered">
          ...
          <tbody>
            <?php
              $sel="SELECT `id`,`sup_name`,`mobile_no`,`company_name`,`company_address`,TRUNCATE(`old_amount`,3) AS old_amount FROM `supplier_setup`";
              ...
              <tr>
                ...
                <td><?php echo $d; ?></td>
                <td><?php echo $e; ?></td>
                <td><?php echo number_format($f,3) ?></td>
              </tr>
              <?php
              $count++;
                }
  ...
  <script>
    ...
    $(document).on('click','#supAdd',function(){

      $('#supAdd').attr("disabled", "disabled");
      $('#supAdd').text("Please Wait !!!");
      ...
      if(fID !="" && supName !="" && mobileNumber !="" && companyName !="" && companyAddress !="" && oldAmount != ""){
        $.ajax({
          url: 'supInsert.php',
          type: "POST",
          data:{
          ...
          success:function(response){
            var data = $.parseJSON(response);
              if(data.status ==  'success'){
                window.location = 'sup_setup.php';
              }else{
                swal(data.message)
                $('#supAdd').removeAttr("disabled");
                $('#supAdd').text("Save");
